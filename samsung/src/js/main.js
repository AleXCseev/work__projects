	...
	card: function() {
		...
		function galary(selector) {
			function toggleActiveSlider(string) {
				var galaryNavFototsSelector = selector + " .slide__nav img";
				var galaryMainFototsSelector = selector + " .main__slide img";
				$(galaryNavFototsSelector).each(function() {
					$(this).hide()
					$(this).attr("src", $(this).data(string)).fadeIn(1000)
				})
				$(galaryMainFototsSelector).each(function() {
					$(this).hide()
					var src =  $(this).data(string)
